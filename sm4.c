...
void WOX(unsigned char data1[], unsigned char data2[], int len)
{
	for (int i = 0; i < len; i++)
	{
...
int Crypt_Enc_Block(unsigned char intput[], int inlen, unsigned char output[], int *outlen, unsigned char key[], int keylen)
{
	*outlen = inlen;
	...
	{
		SM4_Encrypt(key, intput + (16 * i), tmpout);
		memcpy(output + (16 * i), tmpout, 16);
		memset(tmpout, 0x00, 16);
	}
	return 0;
}
int Crypt_Dec_Block(unsigned char intput[], int inlen, unsigned char output[], int *outlen, unsigned char key[], int keylen)
{
	*outlen = inlen;
	...
	for (int i = 0; i < inlen / 16; i++)
	{
		SM4_Decrypt(key, intput + (16 * i), tmpout);
		memcpy(output + (16 * i), tmpout, 16);
		memset(tmpout, 0x00, 16);
	}
	return 0;
...
unsigned char bigIntModMul2(unsigned char *src, int byteLen)
{
	...
	//将整个字节数组左移1位
	for (u32index = 0; u32index < byteLen - 1; u32index++)
	{
		src[u32index] = (src[u32index] << 1);
		src[u32index] |= (src[u32index + 1] >> 7);
	}
	src[byteLen - 1] = (src[byteLen - 1] << 1);

	//若最高位为1，则说明溢出128位，需要与x^128求模
	if (flag)
	{
		src[byteLen - 1] ^= 0x87;
	}
